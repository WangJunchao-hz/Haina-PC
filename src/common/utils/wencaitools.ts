// @ts-nocheck
const TOKEN_SERVER_TIME = new Date().getTime()
...
function init(n, t) {
	...
	let Fn = r[45],
		...
		Kn =
			(c[66] && c[66][f + l]) ||
			(r[51] && r[51].getElementsByTagName(p + d)[r[52]]),
		Qn
	!(function (n) {
		n[e[53]] = s[67]

		function t(n) {
			let t = (r[51] && r[51][u[68]]) || '',
				o = u[69] + n + s[70],
				i = t.indexOf(o)
			if (i == -e[54]) {
	...
	let qn = (function () {
			...
			function l(r) {
				const a = o[102],
					i = e[103]
				this[n[76]] = r
				for (let u = t[52], c = r[a + g + i]; u < c; u++) this[u] = t[52]
			}
			return (
	...
	!(function (n) {
		...
		function G(n) {
			...
			let o, i, u
			o = i = u = r
			for (var c = u[52], v = a[2], f = (n && n[o[56]]) || 0; v < f; v++)
				(c = (c << a[123]) - c + n.charCodeAt(v)), (c >>>= o[52])
			return c
		}

		function K(n) {
			const t = new s[27](e[130], s[80])
			if (n) {
				return n.match(t)
			}
		}
		...
		function Z(n) {
			const t = new u[27](c[154])
			if (M(n)) return n
			const o = t.test(n) ? -a[86] : -parseInt(r[79], e[88])
	...
	!(function (n) {
		...
		function P() {
			const n = v(q, e[171], r[172])
			return null
		}

		function M() {
			for (const n in BROWSER_LIST)
				if (BROWSER_LIST.hasOwnProperty(n)) {
					const t = BROWSER_LIST[n]
					if (t()) return +n.substr(u[0])
				}
			return s[2]
		}
		const O = navigator[s[201]]
		...
		function N(n) {
			if (e[65] && !e[65][at(s[191])]) return !a[54]
			let t
			try {
				t = new a[65][a[155]](n)
			} catch (r) {
		...
		function V() {
			...
			let a, o, i
			a = o = i = u
			for (let c = navigator[o[206]], s = o[2]; s < An[o[111]]; s++)
		...
		function z(n) {
			let t = T
			t = h
			for (let r = a[52]; (r < O && O[u[111]]) || 0; r++) {
				const e = O[r][s[207]]
	...
	!(function (n) {
		...
		function M() {
			const n = Qn.getCookie(Fn) || Zn.get(jn)
	...
	!(function (n) {
		...
		function M() {
			const n = t[51].getElementsByTagName(c[229])
			...
			Jn.eventBind(t[51], r[209], function (n) {
				const t = i[236]
				n = n || event
				let r = n[bn + t + p] || n[Wn(o[5], c[237], o[238])]
				do {
					if (r[i[240]] == i[241]) {
						a(r) && D()
						break
				...
				return function () {
					...
					try {
						X(arguments[o[54]]) && !k(arguments[a[0]])
							...
							X(arguments[r[0]]) ||
								(Qn[u[53]] || (s = rt.update()), this.setRequestHeader(jn, s))
					} catch (v) {
						return o[207]
					}
				}
		...
		function j(n) {
			let r = rn
			r = p
			const e = Jn.getHostFromUrl(n, t[44])
		...
		function V(n) {
			let t = Xn,
				e = o[83][r[232]]
			e && (t = t.concat(e))
			for (let a = i[2]; a < t[c[111]]; a++) if (t[a].test(n)) return o[67]
			return o[86]
		}

		function X(n) {
			const t = Jn.getOriginFromUrl(n)
			return t
				? !new r[47](e[233] + T).test(t[c[122]]) || !new o[27](B).test(t[r[54]])
				: e[61]
		}
		n[t[57]] = O
	})(et || (et = {}))
...
const tools = init(param1, param2)
